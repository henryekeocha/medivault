...
export const syncUserMiddleware = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const email = clerkUser.emailAddresses.find(emailObj => emailObj.id === primaryEmailId)?.emailAddress;
            if (!email) {
            ...
            const role = (clerkUser.publicMetadata?.role || clerkUser.unsafeMetadata?.role || dbUser.role);
            ...
            await prisma.user.update({
                ...
                data: {
                    ...
                    emailVerified: clerkUser.emailAddresses.find(emailObj => emailObj.id === primaryEmailId)?.verification?.status === 'verified'
                        ? new Date()

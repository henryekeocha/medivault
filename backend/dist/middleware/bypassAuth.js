    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            if (bypassHeader && bypassToken && bypassHeader === bypassToken) {
                ...
                const userId = req.headers['x-dev-user-id'] || 'dev-user-123';
                const email = req.headers['x-dev-email'] || 'dev@example.com';
                const roleHeader = req.headers['x-dev-role'] || 'ADMIN';
                // Make sure the role is valid
                const role = (Object.values(Role).includes(roleHeader)
                    ? roleHeader
                    : 'ADMIN');
                req.user = {
                    id: userId,
        ...
        if (req.headers['x-clerk-id']) {
            req.clerkId = req.headers['x-clerk-id'];
        ...
        return protect(req, res, async (err) => {
            if (err) {
...
//# sourceMappingURL=bypassAuth.js.map
...
export class FileService {
    ...
    async uploadFile(file, userId, metadata) {
        try {
            ...
            await Promise.all([
                fs.writeFile(path.join(this.uploadDir, `${fileId}.enc`), encryptedData),
                fs.writeFile(path.join(this.uploadDir, `${fileId}.meta`), JSON.stringify(metadataWithEncryption)),
            ]);
            return {
                fileId,
    ...
    async downloadFile(fileId, encryptionKey) {
        try {

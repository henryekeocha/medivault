...
class CognitoMfaService {
   ...
   */
  async setSmsPreferred(username: string, accessToken?: string): Promise<void> {
    try {
      if (accessToken) {
   ...
   * @param accessToken User's access token (optional)
   */
  async setTotpPreferred(username: string, accessToken?: string): Promise<void> {
    try {
      if (accessToken) {
        // Try with user's access token first
        const setMfaPreferenceCommand = new SetUserMFAPreferenceCommand({
   ...
   * @param accessToken User's access token (optional)
   */
  async disableMfa(username: string, accessToken?: string): Promise<void> {
    try {
      if (accessToken) {
        // Try with user's access token first
        const setMfaPreferenceCommand = new SetUserMFAPreferenceCommand({
   ...
   */
  async requestAttributeVerification(
    accessToken: string,
    attributeName: string
  ): Promise<{destination: string, attributeName: string}> {

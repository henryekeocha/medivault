...
export const syncUser = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const role = (req.body?.role || clerkUser.publicMetadata?.role || clerkUser.unsafeMetadata?.role || 'PATIENT') as Role;
...
async function checkFieldExists(model: string, field: string): Promise<boolean> {
  try {
